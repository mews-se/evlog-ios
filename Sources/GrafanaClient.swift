...
struct GrafanaClient {
    ...
    // the one request shape every query shares - answers arrive as columns
    private func query(_ sql: String) async throws -> Data {
    ...
    func positions(carID: Int, condition: String, sampleSeconds: Int) async throws -> [TrackPoint] {
        ...
        """
        let decoded = try await JSONDecoder().decode(DSResponse.self, from: query(sql))
    ...
    func textColumns(_ sql: String) async throws -> [[String?]] {
        let decoded = try await JSONDecoder().decode(DSTextResponse.self, from: query(sql))
        return decoded.results["A"]?.frames.first?.data.values ?? []
    }

    func scalarText(_ sql: String) async throws -> String? {
        let value = try await textColumns(sql).first?.first ?? nil
